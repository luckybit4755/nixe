...
_wuzzup_git_main() {
	...
	for ((i=0;i<${#};i++)) ; do
		...
		case "${arg}" in
			...
			-catch-up|-cu) _wuzzup_git_catch_up ;;
			-remote) 
				let n=${i}+1
				if [ ${n} -ge ${#} ] ; then 
...
_wuzzup_git_clone_branch() {
	...
	local last=$(echo ${repo} | sed 's,.*/,,;s,\.git$,,')
	local directory=${last}-$( echo ${branch} | sed 's,/,_,g' )
...
_wuzzup_git_new_task() {
	...
	echo -n "do you want to: create a branch called '${branch} on ${repo}/${MAIN} [y/N]? "
	read r
	if [ "y" != "${r}" ] ; then
...
_wuzzup_git_catch_up() {
	local branch=$( git status | head -1 | sed 's,.* ,,' )

	echo "remote origin set to:"
	git remote -v | grep origin
	echo 
	echo -n "do you want catch up to to date with the ${MAIN} branch [y/N]? "
	read r
	if [ "y" = "${r}" ] ; then
		echo --------------------------------------------
